...
## Below is the updated script with bridge-utils replaced by iproute2 commands and a check for other potential issues.
...
echo "Configure network bridge using iproute2"
ip link add name br-ex type bridge
ip link set dev br-ex up
ip link set dev eth0 master br-ex

echo "Verify the bridge configuration"
ip addr show br-ex
ip link show br-ex

echo "Restart networking to apply changes"
